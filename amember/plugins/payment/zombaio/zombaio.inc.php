...
class payment_zombaio extends amember_payment {
	...
	function process_postback($vars){
		...
		if($vars['variable1'] && $vars['X']=='0')
		{
			$payment_id=$vars['variable1'];
			if($vars['Action'] =="user.add" || $vars['Action'] =="rebill")
			{
				$p=$db->get_payment($payment_id);
				$pr=$db->get_product($p['product_id']);
				if (!$p['completed']){
					$err=$db->finish_waiting_payment($p['payment_id'], $this->get_plugin_name(), $vars['TRANSACTION_ID'], '', $vars);
				}
				else
				{
					...
					$newp['receipt_id']  = $vars['TRANSACTION_ID'];
					...
					$err = $db->finish_waiting_payment($new_payment_id, $this->get_plugin_name(), $vars['TRANSACTION_ID'], '', $vars);
				}
