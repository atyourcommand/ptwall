...
';
        ...
        function ProcessXmlData($xml_response) {
            ...
            switch ($message_recognizer) {
                ...
                case "request-received":
                    $this->ProcessRequestReceivedResponse($dom_data_root);
                    break;
        
                // <error> received
                case "error":
                    $this->ProcessErrorResponse($dom_data_root);
                    break;
        
                // <diagnosis> received
                case "diagnosis":
                    $this->ProcessDiagnosisResponse($dom_data_root);
                    break;
        
                // <checkout-redirect> received
                case "checkout-redirect":
                    $this->ProcessCheckoutRedirect($dom_data_root);
                    break;
                /*
                 * +++ CHANGE ME +++
                ...
                case "merchant-calculation-callback":
                    $this->ProcessMerchantCalculationCallback($dom_data_root);
                    break;
                ...
                case "new-order-notification":
                    $this->ProcessNewOrderNotification($dom_data_root);
                    break;
        
                // <order-state-change-notification> received
                case "order-state-change-notification":
                    $this->ProcessOrderStateChangeNotification($dom_data_root);
                    break;
        
                // <charge-amount-notification> received
                case "charge-amount-notification":
                    $this->ProcessChargeAmountNotification($dom_data_root);
                    break;
        
                // <chargeback-amount-notification> received
                case "chargeback-amount-notification":
                    $this->ProcessChargebackAmountNotification($dom_data_root);
                    break;
            
                // <refund-amount-notification> received
                case "refund-amount-notification":
                    $this->ProcessRefundAmountNotification($dom_data_root);
                    break;
        
                // <risk-information-notification> received
                case "risk-information-notification":
                    $this->ProcessRiskInformationNotification($dom_data_root);
                    break;

                // <subscription-request-received> received
                case "subscription-request-received":
                    $this->ProcessRebillingNotification($dom_data_root);
                    break;
                
                // <cancelled-subscription-notification> received
                case "cancelled-subscription-notification":
                    $this->ProcessCancellationNotification($dom_data_root);
                    break;
         ...
         */
        function ProcessRequestReceivedResponse($dom_data_root) {
            /*
             * +++ CHANGE ME +++
         ...
         * @param    $xml_response    synchronous response XML message
         */
        function ProcessErrorResponse($dom_data_root) {
            /*
             * +++ CHANGE ME +++
             * You may need to modify this function if you wish to log
             * information or perform other actions when you receive
         ...
         * @param    $xml_response    synchronous response XML message
         */
        function ProcessDiagnosisResponse($dom_data_root) {
            /*
             * +++ CHANGE ME +++
             * You may need to modify this function if you wish to log
         ...
         * @param    $xml_response    synchronous response XML message
         */
        function ProcessCheckoutRedirect($dom_data_root) {
...
/////        	$this->_tax_tables['alternate-tax-tables'] = $alternate_tax_tables;
        ...
        function ProcessCancellationNotification($dom_data_root) {
            ...
            */
            ...
            $this->SendNotificationAcknowledgment();
        }

        function ProcessRebillingNotification($dom_data_root) {
         ...
         */
        function ProcessNewOrderNotification($dom_data_root) {
            /*
             * +++ CHANGE ME +++
...
//$this->LogMessage ("DEBUG: [".serialize($dom_data_root)."]");
         ...
         * @param    $xml_response    asynchronous notification XML DOM
         */
        function ProcessOrderStateChangeNotification($dom_data_root) {
            /*
             * +++ CHANGE ME +++
         ...
         * @param    $xml_response    asynchronous notification XML DOM
         */
        function ProcessChargeAmountNotification($dom_data_root) {
            /*
             * +++ CHANGE ME +++
         ...
         * @param    $xml_response    asynchronous notification XML DOM
         */
        function ProcessChargebackAmountNotification($dom_data_root) {
            /*
             * +++ CHANGE ME +++
         ...
         * @param    $xml_response    asynchronous notification XML DOM
         */
        function ProcessRefundAmountNotification($dom_data_root) {
            /*
             * +++ CHANGE ME +++
         ...
         * @param    $xml_response    asynchronous notification XML DOM
         */
        function ProcessRiskInformationNotification($dom_data_root) {
            /*
             * +++ CHANGE ME +++
