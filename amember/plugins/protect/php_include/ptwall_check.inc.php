...
*	FileName $RCSfile$
*	Release: 3.1.8PRO ($Revision: 3552 $)
*
      ...
      $today = date('Y-m-d');
			foreach ($pl as $pp){
				if (($pp['begin_date'] <= $today) && ($pp['expire_date'] >= $today))
				{
					...
					if ($products_for_links[$pp['product_id']])
					{
						$begin_date = link_get_first_payment($pp);
						if ($products_for_links[$pp['product_id']]['begin_date'] > $begin_date)
						{
							$products_for_links[$pp['product_id']]['begin_date'] = $begin_date;
						}
					} else {
						$begin_date = link_get_first_payment($pp);
						$products_for_links[$pp['product_id']]['begin_date'] = $begin_date;
			...
			if(defined("INCREMENTAL_CONTENT_PLUGIN"))
			$_SESSION['_amember_links'] = user_get_links($products_for_links);
			if(defined("INCREMENTAL_CONTENT_PLUGIN"))
