...
*    FileName $RCSfile$
*    Release: 3.1.9PRO ($Revision: 4880 $)
*
...
function vsf_address($vars){
    ...
    if (($vars['country'] == 'US' || $vars['country'] == 'CA') && ($vars['state'] == ''))
...
function get_additional_fields_html($vars, $scope, $display_all=0){
...
function member_check_additional_fields(&$vars, $scope='signup'){
    global $member_additional_fields;
    $error = array();
    foreach ($member_additional_fields as $f){
...
function get_member_tax($member_id=null){
    ...
    if ($config['tax_type'] == 1){ // global tax
		...
		if (!$member) return ;
	    foreach ($config['regional_taxes'] as $t){
