...
$config['version'] = '3.1.9PRO';
$config['require_db_version'] = '319';
...
function mail_verification_email($user, $url){
    global $db;
    $t = &new_smarty();
    $t->assign('user',    $user);
    $t->assign('url',     $url);
...
function generate_login($vars='') {
    ...
    if (preg_match("/^([a-zA-Z0-9_]+)\@/", $vars['email'], $regs)){
        ...
        if ($db->check_uniq_login($login) && (strlen($login)>=$config['login_min_length'])){
            return $login;
        }
    }
    ...
    for ($i=1;$i<999;$i++){
        if (strlen($login) > $config['login_max_length'])
            $login = substr($login, 0, $config['login_max_length']);
        $nlogin = $login . $i;
        if ($db->check_uniq_login($nlogin) && (strlen($login)>$config['login_min_length'])){
            return $nlogin;
        }
    }
