...
*    Details: Admin users
*    FileName $RCSfile$
*    Release: 3.1.8PRO ($Revision: 3748 $)
*
    ...
    while (list($d, $cnt, $deb, $cre, $deb_count) = mysql_fetch_row($q)){
        ...
        if ($deb || $cre){
        	$rr = $db->query_all("SELECT c.amount as c_amount, c.payment_id, p.member_id, 
        			...
        			m.login, m.name_f, m.name_l, p.amount as p_amount, c.tier as tier   
...
function edit_save(){
	...
	echo "test xxxxxxxxxxxxxxxxxxxxxxxx users.php?member_id=$vars[member_id]&action=edit";
...
function del_payment(){
	...
	// ptwall code - start
	$temp = md5($oldp['member_id']);
	$url = "http://personaltrainerwall.com/index.php?c=add&m=update_amember_subscription&amember_id=".$oldp['member_id']."&product_id=".$oldp['product_id']."&ref=".$temp."&action=delete";
...
function payment_add(){
	...
	// ptwall code - start
	$temp = md5($vars['member_id']);
	
	$url = "http://personaltrainerwall.com/index.php?c=add&m=update_amember_subscription&amember_id=".$vars['member_id']."&product_id=".$vars['product_id']."&ref=".$temp;
	//echo $url;
	fopen($url, "r");
	// ptwall code - end
