...
function xmlrpc_add_ptwall_user($args){
   
    global $db,$config;
    $u = $args;	
	//return $u;
    $r = $db->add_ptwall_user($u);
	$s = unserialize($u);
	$user = $db->get_user($r);
	$code = substr(uniqid(rand(), true), 0, 12);
	
	
	mail_verification_email($user,  "http://personaltrainerwall.com/index.php?c=welcome&m=amember&member_id=" . $s['login']);
	return $r;
...
$server = & new IXR_Server(array(
    ...
    'add_ptwall_user' => 'xmlrpc_add_ptwall_user',	
