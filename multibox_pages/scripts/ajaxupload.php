<?php
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null, $newName){
		$folder = $relPath;
		$maxlimit = $maxSize;
		...
		if($filesize > 0){	
			...
			foreach($allowed_ext as $ext){
				if($ext==end($file_ext)){
					...
					if ($newName) 
						$newfilename = $newName.".".end($file_ext);
					else
	...
	$newName = strip_tags($_REQUEST['newName']);

	
	$filesize_image = $_FILES[$filename]['size'];
	if($filesize_image > 0){
		$upload_image = uploadImage($filename, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH, $newName);
