...
class Add extends Controller {
		...
		{		
			...
			if ($auth_mode == "facebook") {
				...
				$me['id'] = isset($auth_data['id'])?$auth_data['id']:0;
				$me['name'] =  isset($auth_data['name'])?$auth_data['name']:'';
				$me['first_name'] =  isset($auth_data['first_name'])?$auth_data['first_name']:'';
				$me['middle_name'] =  isset($auth_data['middle_name'])?$auth_data['middle_name']:'';
				$me['last_name'] =  isset($auth_data['last_name'])?$auth_data['last_name']:'';
			...
			// *****************************(Form Object Validation - End)****************************************
					...
					if (isset($_POST['submit'])) {
						...
						if ($user_exist) {
							...
							$page = "Profile Page";
							$subject = "User Update";
							$content = $first_name." ".$last_name." at ".$email." updated their profile";	
						...
						else 
						{
							...
							$last_name = $this->input->post('last_name');
							$email = $this->input->post('email');
							$content = $first_name." ".$last_name." at ".$email." has joined personaltrainerwall.com as a personal trainer";			
							$page = "Profile Page";
							$subject = "New User";
							$this->send_gmail_email($content, $subject, $page, $email);		
					...
					$show_retweets_chkbox = array(
							'name'        => 'show_retweets',
							'value'       => '1',
							'checked'     => $user_data->show_retweets,
							'class'       => 'chk',
							);
					$show_apps_chkbox = array(
							'name'        => 'show_apps',
							'value'       => '1',
							'checked'     => $user_data->show_apps,
							'class'       => 'chk',
							);					
					...
					$data['show_retweets_chkbox'] = $show_retweets_chkbox; 
					$data['show_apps_chkbox'] = $show_apps_chkbox; 
...
*/
  ...
  <status> 
	...
	function remove_profile() {
		...
		$fb_data = _fb_login();
		$auth_data = isset($fb_data['me'])?$fb_data['me']:'';
		$me['id'] = isset($auth_data['id'])?$auth_data['id']:0;
		$logoutUrl = $fb_data['logoutUrl'];
		$uid = isset($fb_data['session']['uid'])?$fb_data['session']['uid']:$me['id'];
		if (isset($_POST['submit']) && ($_POST['submit']!='Cancel')) {
			...
			if (!($user_id=="" || $user_id==0)) {
				...
				$page = "Profile Removal";
				$subject = "Profile Removal";
				...
				redirect('index.php/add/afterRemoveProfile');
				exit();
			} else {
...
/*		$fb_data = _fb_login();
		...
		$auth_data = isset($fb_data['me'])?$fb_data['me']:'';
		$me['id'] = isset($auth_data['id'])?$auth_data['id']:0;
		echo $logoutUrl = $fb_data['logoutUrl'];
		echo $uid = isset($fb_data['session']['uid'])?$fb_data['session']['uid']:$me['id'];
		exit();
		if($uid!="" && $logoutUrl!="")
		{
	...
	function send_gmail_email($content, $subject, $email, $page) {
		...
		$this->email->to('admin@personaltrainerwall.com');
		//$this->email->to('ptwallinvitations@gmail.com');
		//$this->email->to('hasanmarjan@gmail.com');
		$this->email->subject($subject);
